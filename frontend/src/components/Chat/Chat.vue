...
export default {
    ...
    computed:{
        ...mapGetters(['GetUserFollowersFollowing',]),
    ...
    async mounted(){
            ...
            const {data}= await fetchUserProfile(userd.result._id)
            this.MainUserData = data?.user;

        console.log("mainuid", this.MainUserData)
        this.GetUsList();
